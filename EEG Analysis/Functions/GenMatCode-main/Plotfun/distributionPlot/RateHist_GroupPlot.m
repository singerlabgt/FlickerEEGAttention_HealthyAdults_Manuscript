function varargout=RateHist_GroupPlot(X,RateAll,Color,varargin)
    ...
    if nargin==4
        ...
        %% Plot significance bar (will go above the plot
        if Param.statisP==1
            ...
            if strcmp(Param.SigPlot,'Anova')
                if isfield(Param,'Crit_p')
                    crit_p=Param.Crit_p;  % alpha value?
                else
    ...
    %% Plot WPLI for each group
    for iGroup=1:length(RateAll)
        if size(RateAll{iGroup},1)==length(X)
            %     a(iGroup)=error_area(X,nanmean(RateAll{iGroup},2),ste(RateAll{iGroup}'),Color(iGroup,:),0.4);
            ErrorBarPlotLU(X,nanmean(RateAll{iGroup},1),ste(RateAll{iGroup}'),Color(iGroup,:),Param.PlotType,Param.Marker{iGroup},1);
        else
            %     a(iGroup)=error_area(X,nanmean(RateAll{iGroup}),ste(RateAll{iGroup}),Color(iGroup,:),0.4);
            ErrorBarPlotLU(X, nanmean(RateAll{iGroup}),ste(RateAll{iGroup}), Color(iGroup,:), Param.PlotType,Param.Marker{iGroup},1);
        end
        hold on;
    end
...
function stats=ErrorBarPlotLU(x,y_mean,y_errorbar,barColor,PlotMode,Marker,varargin)
    ...
    if (~iscell(y_mean))&&(~isstruct(y_mean))
        ...
        %% Creates Error bar region/cloud
        elseif PlotMode==3
