...
function [r,p] = multiCorr2GroupSubplot(RowNum,ColNum,DataGroup,PairIndex,DataName,Param)
    ...
    for iNode=1:size(PairIndex,2)
        ...
        [OutPut,r,p]= LuPairRegressPlot(DataGroup(:,PairIndex(1,iNode)),DataGroup(:,PairIndex(2,iNode)),Pth,Param);
...
function [OutPut,r,p]=LuPairRegressPlot(data1,data2,Pth,varargin)
    ...
    orange = [1 0.5 0];  % Orange for positive correlation
    indigo = [0.29 0 0.51];  % Indigo for negative correlation
    ...
    if r>0
        PTColor=orange; %[1 0 0];
    elseif r<0
        PTColor=indigo; %[0 0 1];
    else
    end
    ...
    if p < Pth
        ...
        hold on;
        % plot(x,y,'-','color',PColor,'linewidth',2); %pre 9/23/24
        plot(x,y,'-','color',PTColor,'linewidth',2); % MKA 9/23/24 
