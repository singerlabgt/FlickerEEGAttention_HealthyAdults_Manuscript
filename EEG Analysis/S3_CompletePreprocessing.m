...
folderName = ['3_Outputs/Step3_' dateAndTimeAbbr];     %Folder name to save outputs into
...
addChLocs = 1;
applyEventLists = 0;                                     %1 to apply an event list from a file to the datasets, 0 to skip
convertTriggers = 0;                                     % 1 to convert triggers from larger numbers, 0 for not converting triggers
    saveTriggerConvertedData=0;
eventListFileName = 'AttentionTaskEventListStimONLY.txt';        %File name of the events to be analyzed
remove78 = 0;                                            % Remove Channels EXG7 EXG8

filterHighPass = 0;                                      %1 to perform a high-pass filter on the data, 0 to skip
...
extractEpochsEarly = 1;                                  %1 to extract epochs early (BEFORE bad channel removal)
...
%% Artifact Detection Steps
    artifactDetection = 1;                                   %1 to perform artifact detection on the epochs, 0 to skip
...
disp([scriptName ' is done running']);